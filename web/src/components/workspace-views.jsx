...
function LoginView({ workspace, onLogin, onSettings }) {
  const [accounts, setAccounts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [assigning, setAssigning] = useState(null);

  useEffect(() => {
    fetch(`${getApiBase()}/api/accounts`, { headers: getAuthHeaders() })
      .then(r => r.json())
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full gap-6 p-8">
      ...
      <div className="text-center space-y-2">
        <h2 className="text-lg font-medium text-white">Select Account</h2>
        <p className="text-sm text-zinc-400 max-w-sm">
          Choose a Google account to use with this workspace.
        </p>
      </div>
      {accounts.length > 0 ? (
        <div className="w-full max-w-sm space-y-1.5">
          {accounts.map(a => (
            <button
              key={a._id}
              onClick={() => assignAccount(a._id)}
              disabled={assigning === a._id}
              className="w-full flex items-center gap-3 px-4 py-3 bg-zinc-900 border border-white/5 rounded-lg hover:border-blue-500/30 hover:bg-zinc-800 transition-all text-left group"
            >
              {a.avatar ? (
                <img src={a.avatar} className="w-8 h-8 rounded-full shrink-0" alt="" />
              ) : (
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium text-white truncate">{a.name || a.email}</div>
                <div className="text-[11px] text-zinc-500 truncate">{a.email}</div>
              </div>
              {assigning === a._id ? (
                <Loader2 className="w-4 h-4 animate-spin text-blue-400 shrink-0" />
              ) : (
                <Play className="w-3.5 h-3.5 text-zinc-600 group-hover:text-blue-400 shrink-0" />
              )}
            </button>
          ))}
          <div className="pt-2 flex gap-2">
            <button
              onClick={onLogin}
              className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-zinc-800 border border-white/10 text-zinc-300 rounded-lg text-xs font-medium hover:bg-zinc-700 transition-colors"
            >
        ...
        <div className="w-full max-w-sm space-y-3">
          <div className="text-center py-4">
            <p className="text-xs text-zinc-500 mb-4">No Google accounts configured yet.</p>
          </div>
          <button
            onClick={onLogin}
            className="w-full flex items-center justify-center gap-2 px-5 py-2.5 bg-white text-black rounded-lg font-medium text-sm hover:bg-zinc-200 transition-colors"
          >
          ...
          {onSettings && (
            <button
              onClick={onSettings}
              className="w-full flex items-center justify-center gap-2 px-4 py-2 text-zinc-400 hover:text-white text-xs transition-colors"
            >
              Or manage accounts in Settings
            </button>
          )}
        </div>
      )}
